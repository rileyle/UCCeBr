...
//============================================
	...
	//Uncomment the block below to implement the full scattering chamber
	
	/*auto ss_mesh = CADMesh::TessellatedMesh::FromSTL("./assembly_chamber_02_ascii.stl");
	...
	//Logical Volume
	// Even if you implement the scattering chamber, it is still called spherical shell here. 
	...
	// Change to the parameters below for the scattering chamber

	/*G4double ss_x = 27.0*cm;
	G4double ss_y = 27.0*cm;
	G4double ss_z = 33.0*cm;
...
//======================//	
	...
	DetectorAssembly* Det0 = new DetectorAssembly();
	DetectorAssembly* Det1 = new DetectorAssembly();
	DetectorAssembly* Det2 = new DetectorAssembly();
	DetectorAssembly* Det3 = new DetectorAssembly();

	Det0->SetDetFront(detFront_2x2);
	Det0->SetDetBack(detBack_2x2);

	Det0->ConstructAssembly();

	Det1->SetDetFront(detFront_2x2);
	Det1->SetDetBack(detBack_2x2);

	Det1->ConstructAssembly();

	Det2->SetDetFront(detFront_2x2);
	Det2->SetDetBack(detBack_2x2);

	Det2->ConstructAssembly();

	Det3->SetDetFront(detFront_2x2);
	Det3->SetDetBack(detBack_2x2);

	Det3->ConstructAssembly();
	...
	DetectorAssembly* Det4 = new DetectorAssembly();

	Det4->SetDetFront(detFront_3x4);
	Det4->SetDetBack(detBack_3x4);

	Det4->ConstructAssembly();
...
//===========================================
	...
	//Placement Variables
	G4double gap0;		//Distance of detector from apparatus
	G4double gap1;
	G4double gap2;
	G4double gap3;
	G4double gap4;
...
//	gap = 14.0*mm;
	//gap2 = 5.0*in - ss_outRad;
	gap0 = 4.3*in - ss_outRad;
	gap1 = 4.3*in - ss_outRad;
	gap2 = 4.3*in - ss_outRad;
	gap3 = 4.3*in - ss_outRad;
	...
	Det0->PlaceAssembly(worldLog, ss_outRad, gap0, 211*deg, phi, spin);
	Det1->PlaceAssembly(worldLog, ss_outRad, gap1, 241*deg, phi, spin);
	Det2->PlaceAssembly(worldLog, ss_outRad, gap2, 271*deg, phi, spin);
	Det3->PlaceAssembly(worldLog, ss_outRad, gap3, 96*deg, phi, spin);

	//Detector 1
	
//	gap = 19.0*mm;
	gap4 = 4.3*in - ss_outRad;
	...
	Det4->PlaceAssembly(worldLog, ss_outRad, gap4, 141*deg, phi, spin);
